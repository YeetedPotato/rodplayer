...
void main() {
  ...
  Widget harness({bool visible = true}) {
    return MaterialApp(
  ...
  testWidgets('traverses volume, seek, play, and settings controls', (tester) async {
    ...
    await tester.pump();
    expect(tester.widget<IconButton>(volume).focusNode?.hasFocus ??
        Focus.of(tester.element(volume)).hasFocus, isTrue);
  ...
  testWidgets('DPAD arrows adjust seek and select activates playback', (tester) async {
    ...
    await tester.pump();
    expect(engine.player.state.playing, isFalse);
  ...
  testWidgets('focused HUD controls expose Brand Gold focus styling', (tester) async {
    await tester.pumpWidget(harness());
    await tester.pump();
    ...
    final theme = Theme.of(tester.element(settings)).extension<RemuxTheme>()!;
