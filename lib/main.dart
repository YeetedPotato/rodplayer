...
Future<void> main() async { WidgetsFlutterBinding.ensureInitialized(); MediaKit.ensureInitialized(); runApp(RemuxApp(preferences: await SharedPreferences.getInstance())); }
class RemuxApp extends StatelessWidget { const RemuxApp({required this.preferences, super.key}); final SharedPreferences preferences; @override Widget build(BuildContext context) => MaterialApp(title: 'Remux', theme: remuxThemeData(), home: RemuxShell(preferences: preferences)); }
class RemuxShell extends StatefulWidget { const RemuxShell({required this.preferences, super.key}); final SharedPreferences preferences; @override State<RemuxShell> createState() => _RemuxShellState(); }
class _RemuxShellState extends State<RemuxShell> { RemuxClient? _client; bool _loading = true;
...
Widget playerRoute(RemuxEngine engine, RemuxClient client, String itemId)=>VideoPlayerView(engine:engine,client:client,itemId:itemId);
