...
class PlaybackStreamInfo {
  const PlaybackStreamInfo({required this.itemId, required this.mediaSourceId, required this.playMethod, this.decisionReason});
  final String itemId;
...
class RemuxClient {
  ...
  Future<void> authenticate({required String username, required String password}) async {
    ...
    if (response.statusCode == 401 || response.statusCode == 403) throw RemuxAuthException('Jellyfin rejected the supplied credentials (${response.statusCode})');
    _check(response);
  ...
  Future<List<dynamic>> getNextUp({int limit = 12}) async => _items(await _client.get(Uri.parse('$baseUrl/Shows/NextUp?${_userQuery()}&Limit=$limit&Fields=PrimaryImageAspectRatio,UserData,SeriesName,SeasonNumber,IndexNumber,RunTimeTicks'), headers: _headers));
  ...
  Future<void> toggleFavorite({required String itemId, required bool isFavorite}) async {
    ...
    final response = isFavorite ? await _client.post(endpoint, headers: _headers) : await _client.delete(endpoint);
    _check(response);
  }

  Future<PlaybackStreamInfo> getStreamInfo(String itemId) async {
    ...
    for (final value in sources) {
      ...
      if (decision.url == null || decision.method == PlayMethod.transcode) continue;
      return PlaybackStreamInfo(itemId: itemId, mediaSourceId: '${value['Id'] ?? itemId}', playMethod: decision.method.name, decisionReason: decision.reason);
    }
  ...
  Future<Uri> getStreamUri(String itemId) async {
    ...
    for (final value in sources) {
      ...
      if (url == null || decision.method == PlayMethod.transcode) continue;
      final resolved = url.hasScheme ? url : Uri.parse(baseUrl).resolve(url.toString());
      final token = _cleanToken(accessToken);
      return token != null && !resolved.queryParameters.containsKey('api_key') ? resolved.replace(queryParameters: {...resolved.queryParameters, 'api_key': token}) : resolved;
    }
    throw RemuxConnectionException('Remux returned no playable media source');
  }

  Future<void> reportPlaybackStarted({required String itemId, required String sessionId, String playMethod = 'DirectPlay', String? mediaSourceId}) async {
    this.sessionId = sessionId;
    final response = await _client.post(Uri.parse('$baseUrl/Sessions/Playing'), headers: _headers, body: jsonEncode({'ItemId': itemId, 'SessionId': sessionId, 'MediaSourceId': mediaSourceId ?? itemId, 'PlayMethod': playMethod, 'IsPaused': false}));
    _check(response);
  }

  Future<void> reportPlaybackProgress({required String itemId, required Duration position, required Duration duration, bool isPaused = false, String playMethod = 'DirectPlay', String? mediaSourceId}) async {
    final activeSession = sessionId;
    if (activeSession == null) return;
    final response = await _client.post(Uri.parse('$baseUrl/Sessions/Playing/Progress'), headers: _headers, body: jsonEncode({'ItemId': itemId, 'SessionId': activeSession, 'PositionTicks': position.inMicroseconds * 10, 'MediaSourceId': mediaSourceId ?? itemId, 'IsPaused': isPaused, 'PlayMethod': playMethod, 'EventName': 'timeupdate', 'RunTimeTicks': duration.inMicroseconds * 10}));
    _check(response);
  }

  Future<void> reportPlaybackStopped({required String itemId, required Duration position, String playMethod = 'DirectPlay', String? mediaSourceId}) async {
    final activeSession = sessionId;
    if (activeSession == null) return;
    final response = await _client.post(Uri.parse('$baseUrl/Sessions/Playing/Stopped'), headers: _headers, body: jsonEncode({'ItemId': itemId, 'SessionId': activeSession, 'MediaSourceId': mediaSourceId ?? itemId, 'PositionTicks': position.inMicroseconds * 10, 'PlayMethod': playMethod}));
    _check(response);
    sessionId = null;
  }
  ...
  void _check(http.Response response) {
    if (response.statusCode < 200 || response.statusCode >= 300) throw RemuxConnectionException('Remux request failed (${response.statusCode})', statusCode: response.statusCode);
  }
