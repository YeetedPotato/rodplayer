...
class TrackSelectorSheet extends StatefulWidget {
  ...
  static Future<void> show(BuildContext context, {required Player player}) {
    return showModalBottomSheet<void>(
      ...
      builder: (_) => TrackSelectorSheet(player: player),
    );
  }
...
class _TrackSelectorSheetState extends State<TrackSelectorSheet> {
  ...
  String _trackLabel(String? title, String? language, String? codec) {
    final parts = <String>[title, language, codec]
        .whereType<String>()
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        _TrackGroup<AudioTrack>(
                          ...
                          label: (track) => _trackLabel(track.title, track.language, track.codec),
                        ...
                        _TrackGroup<SubtitleTrack>(
                          ...
                          onOff: () => _selectSubtitle(SubtitleTrack.no()),
                        ),
                      ],
                    ),
                  ),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        if (tracks.isEmpty)
          Text('No tracks available', style: TextStyle(color: theme.textMuted))
        else
          ...tracks.map((track) => _TrackTile(label: label(track), selected: track == selected, onTap: () => onSelected(track))),
            ...
            child: InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(theme.radiusSmall),
                  border: Border.all(color: focused ? theme.goldBright : Colors.transparent, width: 2),
                ),
