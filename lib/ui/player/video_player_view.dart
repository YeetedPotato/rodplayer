...
class _VideoPlayerViewState extends State<VideoPlayerView> {
  Timer? _keepAlive;
  @override void initState() { super.initState(); widget.engine.error.addListener(_showPlaybackError); unawaited(widget.playbackSession?.begin()); _keepAlive = Timer.periodic(const Duration(seconds: 10), (_) => unawaited(_report())); }
  ...
  Future<void> _report() async { final id = widget.itemId; final session = widget.playbackSession; final playerState = widget.engine.player.state; if (session != null) { await session.reportProgress(playerState.position, playerState.duration, paused: !playerState.playing); return; } if (id == null) return; await widget.client.reportProgress(itemId: id, position: playerState.position, duration: playerState.duration, isPaused: !playerState.playing); }
  @override void dispose() { widget.engine.error.removeListener(_showPlaybackError); _keepAlive?.cancel(); final playerState = widget.engine.player.state; final session = widget.playbackSession; if (session != null) { unawaited(session.end(playerState.position)); } else { unawaited(_report()); } super.dispose(); }
