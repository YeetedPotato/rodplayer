...
class _VideoPlayerViewState extends State<VideoPlayerView> {
  Timer? _keepAlive;
  @override
  void initState() { super.initState(); widget.engine.error.addListener(_showPlaybackError); _keepAlive = Timer.periodic(const Duration(seconds: 10), (_) => unawaited(_report())); }
  ...
  Future<void> _report() async { final id = widget.itemId; if (id == null) return; final p = widget.engine.player.state; await widget.client.reportProgress(itemId: id, position: p.position, duration: p.duration, isPaused: !p.playing); }
  ...
  @override
  Widget build(BuildContext context) => PopScope(canPop: true, child: FocusTraversalGroup(policy: OrderedTraversalPolicy(), child: Focus(autofocus: true, onKeyEvent: (_, event) { if (event.logicalKey.keyLabel == 'Media Play Pause') widget.engine.player.playOrPause(); return KeyEventResult.ignored; }, child: Scaffold(backgroundColor: Colors.black, body: Stack(children: [Center(child: Video(controller: widget.engine.controller, controls: AdaptiveVideoControls)), Positioned(top: 20, left: 20, child: _Hud(engine: widget.engine))])))));
}
