...
class _BrowseScreenState extends State<BrowseScreen> {
  ...
  final List<String> _tabs = const [
    'Home', 'Movies', 'TV Shows', 'Music', 'Live TV', 'Collections',
  ];
  int _tab = 0;
    ...
    widget.client.getItems().catchError((Object error) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Unable to load library: $error')),
        );
      }
      return <dynamic>[];
    });
  ...
  Widget _shelf(String title, List<String> names, {double ratio = 2 / 3}) => SmartShelf(
    ...
    itemBuilder: (_, int index) => _card(names[index], index, ratio: ratio),
  );
    ...
    showModalBottomSheet<void>(
      ...
      builder: (_) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const Text('A premium Jellyfin media experience with Remux playback intelligence.', style: TextStyle(color: Colors.white70)),
          const SizedBox(height: 16),
          MediaBadges(labels: const ['4K', 'HDR10+', 'DV', 'Atmos']),
          ...
          const SizedBox(height: 8),
          const Text('Continue watching, explore seasons, cast and crew, or inspect direct-play compatibility from this detail surface.', style: TextStyle(color: Colors.white70)),
          const Spacer(),
        ]),
      ),
    );
            ...
            slivers: [
              SliverAppBar(
                ...
                actions: [
                  ...
                  const Padding(padding: EdgeInsets.symmetric(horizontal: 12), child: CircleAvatar(child: Text('R'))),
                ],
                bottom: PreferredSize(
                  preferredSize: const Size.fromHeight(48),
                  child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
                    for (int index = 0; index < _tabs.length; index++)
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: ChoiceChip(label: Text(_tabs[index]), selected: _tab == index, onSelected: (_) => setState(() => _tab = index))),
                  ])),
                ),
              ),
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(borderRadius: BorderRadius.circular(22), gradient: LinearGradient(colors: [theme.green, theme.surface2]), boxShadow: theme.glassShadow),
                    child: Column(mainAxisAlignment: MainAxisAlignment.end, crossAxisAlignment: CrossAxisAlignment.start, children: [
                      const Text('Featured collection', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w800)),
                      const SizedBox(height: 10),
                      MediaBadges(labels: const ['4K', 'HDR10+', 'Atmos', 'HEVC']),
                      const SizedBox(height: 14),
                      const Row(children: [
                        FilledButton.icon(onPressed: null, icon: Icon(Icons.play_arrow), label: Text('Play')),
                        SizedBox(width: 8),
                        OutlinedButton.icon(onPressed: null, icon: Icon(Icons.shuffle), label: Text('Surprise Me')),
                      ]),
                    ]),
                  ),
                ),
