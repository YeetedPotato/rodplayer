...
class _BrowseScreenState extends State<BrowseScreen> {
  ...
  String? _imageUrl(dynamic item) {
    ...
    final id = item['Id'];
    final images = item['ImageTags'];
    final tag = images is Map<String, dynamic> ? images['Primary'] : null;
  ...
  Widget _card(String title, int index, {double ratio = 2 / 3, String? imageUrl}) => GestureDetector(
    onLongPress: () => showQuickActionsSheet(context, title: title),
    child: FocusableMediaCard(
      title: title,
      subtitle: ratio > 1 ? 'Episode ${index + 1}' : 'Library',
      imageUrl: imageUrl ?? _posters[index % _posters.length],
      aspectRatio: ratio,
  ...
  Widget _dataShelf(String title, Future<List<dynamic>> future, {double ratio = 2 / 3}) => FutureBuilder<List<dynamic>>(
    future: future,
    builder: (context, snapshot) {
      if (snapshot.connectionState == ConnectionState.waiting) {
        return _statusShelf(title, 'Loading…');
      }
      final items = snapshot.data ?? const <dynamic>[];
      if (snapshot.hasError || items.isEmpty) return _statusShelf(title, 'Nothing here yet');
      return SmartShelf(
        title: title, itemCount: items.length, aspectRatio: ratio, itemCountBadge: true,
        autofocusFirstItem: true,
        itemBuilder: (_, index) => _card(_title(items[index]), index, ratio: ratio, imageUrl: _imageUrl(items[index])),
      );
    },
  );

  Widget _statusShelf(String title, String message) => Padding(
    padding: const EdgeInsets.fromLTRB(24, 18, 24, 24),
    child: DecoratedBox(
      decoration: BoxDecoration(color: Colors.black, borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFF333333))),
      child: Padding(padding: const EdgeInsets.all(20), child: Row(children: [Text(title, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700)), const Spacer(), Text(message, style: const TextStyle(color: Colors.white70))])),
    ),
  );
    ...
    showModalBottomSheet<void>(
      ...
      builder: (_) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Spacer(), Text(title, style: const TextStyle(fontSize: 30, fontWeight: FontWeight.w800)), const SizedBox(height: 8),
          const Text('A premium Jellyfin media experience with Remux playback intelligence.', style: TextStyle(color: Colors.white70)), const SizedBox(height: 16),
          MediaBadges(labels: const ['4K', 'HDR10+', 'DV', 'Atmos']), const SizedBox(height: 18),
          Row(children: [FilledButton.icon(onPressed: () {}, icon: const Icon(Icons.play_arrow), label: const Text('Play')), IconButton(onPressed: () {}, icon: const Icon(Icons.favorite_border)), IconButton(onPressed: () {}, icon: const Icon(Icons.more_horiz))]),
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(canPop: true, child: Scaffold(
      ...
      drawer: Drawer(backgroundColor: theme.surface2, child: SafeArea(child: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        for (final String item in ['Home', 'Libraries', 'Settings', 'Server Info']) ListTile(leading: Icon(item == 'Home' ? Icons.home_outlined : Icons.folder_outlined), title: Text(item), onTap: () => Navigator.pop(context)),
      ...
      body: FocusTraversalGroup(policy: OrderedTraversalPolicy(), child: CustomScrollView(controller: _scroll, slivers: [
        SliverAppBar(pinned: true, backgroundColor: theme.obsidianGlassStrong, surfaceTintColor: Colors.transparent,
          ...
          bottom: PreferredSize(preferredSize: const Size.fromHeight(48), child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [for (int index = 0; index < _tabs.length; index++) Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: ChoiceChip(label: Text(_tabs[index]), selected: _tab == index, onSelected: (_) => setState(() => _tab = index)))]))),
        ),
        SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(24, 24, 24, 8), child: Container(height: 300, padding: const EdgeInsets.all(24), decoration: BoxDecoration(borderRadius: BorderRadius.circular(22), gradient: LinearGradient(colors: [theme.gold, theme.obsidian]), boxShadow: theme.glassShadow), child: Column(mainAxisAlignment: MainAxisAlignment.end, crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Featured collection', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w800)), const SizedBox(height: 10), MediaBadges(labels: const ['4K', 'HDR10+', 'Atmos', 'HEVC']), const SizedBox(height: 14), Row(children: [FilledButton.icon(onPressed: null, icon: const Icon(Icons.play_arrow), label: const Text('Play')), const SizedBox(width: 8), OutlinedButton.icon(onPressed: null, icon: const Icon(Icons.shuffle), label: const Text('Surprise Me'))])])))),
