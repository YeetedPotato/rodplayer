...
class _BrowseScreenState extends State<BrowseScreen> {
  ...
  Widget _card(String title, int index, {double ratio = 2 / 3, String? imageUrl, String? itemId}) => GestureDetector(onLongPress: () => showQuickActionsSheet(context, title: title), child: FocusableMediaCard(title: title, subtitle: ratio > 1 ? 'Episode ${index + 1}' : 'Library', imageUrl: imageUrl ?? _posters[index % _posters.length], aspectRatio: ratio, mediaInfo: const MediaIntelligence(width: 3840, height: 2160, video: StreamIntelligence(type: 'Video', codec: 'hevc', videoRange: 'HDR10+'), audio: StreamIntelligence(type: 'Audio', codec: 'truehd', channels: 8, channelLayout: '7.1')), onTap: () => _showDetails(title, itemId: itemId)));
  ...
  Widget _dataShelf(String title, Future<List<dynamic>> future, {double ratio = 2 / 3}) => FutureBuilder<List<dynamic>>(future: future, builder: (context, snapshot) { if (snapshot.connectionState == ConnectionState.waiting) return _statusShelf(title, 'Loading…'); final items = snapshot.data ?? const <dynamic>[]; if (snapshot.hasError || items.isEmpty) return _statusShelf(title, 'Nothing here yet'); return SmartShelf(title: title, itemCount: items.length, aspectRatio: ratio, itemCountBadge: true, autofocusFirstItem: true, itemBuilder: (_, index) { final item = items[index]; final itemId = item is Map<String, dynamic> && item['Id'] is String ? item['Id'] as String : null; return _card(_title(item), index, ratio: ratio, imageUrl: _imageUrl(item), itemId: itemId); }); });
  ...
  void _playItem(String itemId) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Playing item $itemId'))); }
  void _showDetails(String title, {String? itemId}) => showModalBottomSheet<void>(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (_) => Container(height: MediaQuery.sizeOf(context).height * .78, padding: const EdgeInsets.all(24), decoration: const BoxDecoration(color: Color(0xFF0D0F12), borderRadius: BorderRadius.vertical(top: Radius.circular(24))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Spacer(), Text(title, style: const TextStyle(fontSize: 30, fontWeight: FontWeight.w800)), const SizedBox(height: 8), const Text('A premium Jellyfin media experience with Remux playback intelligence.', style: TextStyle(color: Colors.white70)), const SizedBox(height: 16), MediaBadges(labels: const ['4K', 'HDR10+', 'DV', 'Atmos']), const SizedBox(height: 18), Row(children: [FilledButton.icon(onPressed: itemId == null ? null : () => _playItem(itemId), icon: const Icon(Icons.play_arrow), label: const Text('Play')), IconButton(onPressed: () {}, icon: const Icon(Icons.favorite_border)), IconButton(onPressed: () {}, icon: const Icon(Icons.more_horiz))]), const Text('Overview', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)), const SizedBox(height: 8), const Text('Continue watching, explore seasons, cast and crew, or inspect direct-play compatibility from this detail surface.', style: TextStyle(color: Colors.white70)), const Spacer()])));
